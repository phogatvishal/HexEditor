...
namespace LFT_Hex_Editor
{
    public partial class CompareWindow : Window
    {
        ...
        private void FindDifference()
        {
            ...
            // Create a HashSet to store all the byte positions in _differences for fast lookup
            HashSet<long> bytePositions = new HashSet<long>(_differences.Select(d => d.BytePositionInStream));
            ...
            foreach (var byteDifference in _differences)
            {
                ...
                // Check if the next byte position exists in the bytePositions set
                if (!bytePositions.Contains(byteDifference.BytePositionInStream + 1))
                {
                    j = 0;
            ...
            {
                MessageBox.Show("No differences are found!");
            }
